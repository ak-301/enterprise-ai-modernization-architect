...
Document (and optionally deploy) Azure mapping of local stack.

## 3. Why are we building it?

Interview signal for cloud modernization on Azure.

## 4. What problem does it solve?

Local-only story with no cloud path.

## 5. Concepts I need to understand first

### Managed identity

**Simple:** See [GLOSSARY.md](../GLOSSARY.md) for `Managed identity` if listed; otherwise learn it while implementing this stage.

### Key Vault

**Simple:** See [GLOSSARY.md](../GLOSSARY.md) for `Key Vault` if listed; otherwise learn it while implementing this stage.

### Private networking (future)

**Simple:** See [GLOSSARY.md](../GLOSSARY.md) for `Private networking (future)` if listed; otherwise learn it while implementing this stage.


## 6. Technologies used

Primary stack for this stage: **Azure services**
...
- **How we use it:** Managed Postgres
...
Inputs from earlier stages
        ↓
Stage 19 processing (Azure Deployment Architecture)
        ↓
...
- This document: `docs/stages/STAGE_19.md`
...
1. What is Stage 19 trying to produce?
...
1. “It produces azure deployment architecture capabilities that later stages consume.”
...
“Stage 19 is Azure Deployment Architecture. It isn't built yet in the repo. When we implement it, it will sit in the pipeline between earlier data/platform work and later recommendation/governance stages.”

## 19. 2-minute explanation

“In the full project design, Stage 19 exists because Interview signal for cloud modernization on Azure. Today the status is planned only — I'm careful not to claim it in interviews as shipped. The learning goal is to understand the problem it solves: Local-only story with no cloud path. Technologies we expect: Azure services.”
...
- Status: **PLANNED**
- Goal: Document (and optionally deploy) Azure mapping of local stack.
- Why: Interview signal for cloud modernization on Azure.
- Key tech: Azure services
