...
Adversarial cases: missing data, conflicts, tool/DB/LLM failures, invalid JSON.
...
**Simple:** See [GLOSSARY.md](../GLOSSARY.md) for `Fail-safe` if listed; otherwise learn it while implementing this stage.

### Retry/repair

**Simple:** See [GLOSSARY.md](../GLOSSARY.md) for `Retry/repair` if listed; otherwise learn it while implementing this stage.

### Degradation

**Simple:** See [GLOSSARY.md](../GLOSSARY.md) for `Degradation` if listed; otherwise learn it while implementing this stage.


## 6. Technologies used

Primary stack for this stage: **pytest failure harness**

### Integration tests

- **What it is:** Chaos-ish cases

- **Why we use it:** Fits this stage's problem without overengineering.

- **How we use it:** Assert safe behavior
...
Inputs from earlier stages
        ↓
Stage 15 processing (Failure & Resilience)
        ↓
...
- This document: `docs/stages/STAGE_15.md`
...
1. What is Stage 15 trying to produce?
...
1. “It produces failure & resilience capabilities that later stages consume.”
...
“Stage 15 is Failure & Resilience. It isn't built yet in the repo. When we implement it, it will sit in the pipeline between earlier data/platform work and later recommendation/governance stages.”

## 19. 2-minute explanation

“In the full project design, Stage 15 exists because Production AI must fail safely. Today the status is planned only — I'm careful not to claim it in interviews as shipped. The learning goal is to understand the problem it solves: Happy-path-only demos. Technologies we expect: pytest failure harness.”
...
- Status: **PLANNED**
- Goal: Adversarial cases: missing data, conflicts, tool/DB/LLM failures, invalid JSON.
- Why: Production AI must fail safely.
- Key tech: pytest failure harness
