...
Pipeline: raw → parse → validate → normalize → dedupe → enrich → store + quality report.
...
Silent acceptance of bad rows and missing quality metrics.

## 5. Concepts I need to understand first

### ETL

**Simple:** See [GLOSSARY.md](../GLOSSARY.md) for `ETL` if listed; otherwise learn it while implementing this stage.

### Validation error

**Simple:** See [GLOSSARY.md](../GLOSSARY.md) for `Validation error` if listed; otherwise learn it while implementing this stage.

### Completeness

**Simple:** See [GLOSSARY.md](../GLOSSARY.md) for `Completeness` if listed; otherwise learn it while implementing this stage.

### Referential integrity

**Simple:** See [GLOSSARY.md](../GLOSSARY.md) for `Referential integrity` if listed; otherwise learn it while implementing this stage.


## 6. Technologies used

Primary stack for this stage: **Python, Pydantic, PostgreSQL**

### Pydantic

- **What it is:** Row validation

- **Why we use it:** Fits this stage's problem without overengineering.

- **How we use it:** Reject invalid records
...
- **How we use it:** Load clean inventory
...
Inputs from earlier stages
        ↓
Stage 4 processing (Data Engineering Pipeline)
        ↓
...
- This document: `docs/stages/STAGE_04.md`
...
1. What is Stage 4 trying to produce?
...
1. “It produces data engineering pipeline capabilities that later stages consume.”
...
“Stage 4 is Data Engineering Pipeline. It isn't built yet in the repo. When we implement it, it will sit in the pipeline between earlier data/platform work and later recommendation/governance stages.”

## 19. 2-minute explanation

“In the full project design, Stage 4 exists because Dirty inventory must not silently become AI 'truth'. Today the status is planned only — I'm careful not to claim it in interviews as shipped. The learning goal is to understand the problem it solves: Silent acceptance of bad rows and missing quality metrics. Technologies we expect: Python, Pydantic, PostgreSQL.”
...
- Status: **PLANNED**
- Goal: Pipeline: raw → parse → validate → normalize → dedupe → enrich → store + quality report.
- Why: Dirty inventory must not silently become AI 'truth'.
- Key tech: Python, Pydantic, PostgreSQL
